    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-start">
                ...
                <div
                    className={`flex flex-col items-center px-5 py-3 rounded-xl border font-mono font-bold shadow-inner ${limitReached                         ? 'bg-red-950/40 border-red-700 text-red-300'                         : totalPlayedThisRound >= maxCardsPerRound - 1                             ? 'bg-amber-950/40 border-amber-700 text-amber-300'                             : 'bg-zinc-800/60 border-zinc-700 text-zinc-300'                         }`}
                >
                    ...
                    <span className="text-[10px] uppercase tracking-widest mt-1 font-semibold opacity-70">
                        Eventos / Rodada (Mestre ilimitado)
                    </span>
                </div>
            </div>
            ...
            <section className="bg-zinc-800/40 border border-zinc-700/50 rounded-xl p-5 space-y-4">
                ...
                <button className="w-full sm:w-auto px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition-colors disabled:opacity-50"
                    onClick={handleDrawRandom} disabled={loading}>
            ...
            <section className="space-y-4 pt-4 border-t border-zinc-700/50">
                <h3 className="font-semibold text-lg text-zinc-100">
                    📍 Acionar Evento Específico Manualmente
                </h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                    {templates.map(t => (
                        <div
                            key={t.id}
                            className={`p-4 rounded-xl border flex flex-col gap-3 ${t.isPositive
                        ...
                        >
                            <div className="flex items-start justify-between gap-2">
                                <div>
                                    ...
                                    <p className="text-xs font-mono text-zinc-400">
                                        Scope: {t.scope} | Prob: {t.probability}
                                    </p>
                                </div>
                                <button
                                    onClick={() => handlePlaySpecific(t.id)}
                                    disabled={loading}
                                    className={`px-3 py-1.5 text-xs font-bold rounded shadow transition-colors whitespace-nowrap ${t.isPositive
                            ...
                            <div className="text-xs text-zinc-400 border-l-2 border-zinc-700 pl-2">
                                <span className="font-semibold text-blue-400">Análise (Guia): </span>
                                {t.expertAnalysis}
                            </div>
                        </div>
                    ))}
                    {templates.length === 0 && (
                        <p className="text-zinc-500 text-sm">Carregando templates do banco de dados...</p>
                    )}
                </div>
            </section>
