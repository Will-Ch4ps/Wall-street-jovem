...
export function CardsTab({ state, onCardPlayed }: CardsTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
          {Object.entries(groupedCards).map(([key, group]) => {
            ...
            return (
              <div key={key} className={`rounded-xl border p-4 ${group.color}`}>
                <h4 className="font-bold text-sm mb-3 border-b border-current/20 pb-2">
                  {group.label}
                </h4>
                <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                  {group.cards.map((c) => (
                    <div
                      key={c.id}
                      className="bg-black/20 rounded-lg p-2.5 hover:bg-black/30 transition group"
                    >
                      <div className="flex justify-between items-start mb-1">
                        <span className="font-semibold text-xs leading-tight">{c.name}</span>
                        <button
                          onClick={() => handlePlaySpecific(c.id)}
                          disabled={loading || limitReached}
                          title={
                            limitReached
                              ? `Limite de ${maxCardsPerRound} cartas/rodada atingido`
                              : 'Jogar esta carta'
                          }
                          className="text-[10px] font-bold bg-white/10 hover:bg-white/20 text-white rounded px-2 py-1 opacity-0 group-hover:opacity-100 transition disabled:opacity-30 disabled:cursor-not-allowed"
                        >
                          {limitReached ? '🔒' : 'Jogar'}
                        </button>
                      </div>
                      ...
                      {c.newsTemplate && (
                        <p className="text-[9px] mt-1.5 font-mono opacity-60">
                          📰 Gera notícia ({c.newsTemplate})
                        </p>
                      )}
                    </div>
                  ))}
      ...
      {/* ── Histórico de cartas desta rodada ─────────────────────── */}
      {state.activeCardEffects.filter((e) => e.round === currentRound).length > 0 && (
          ...
          <div className="space-y-1">
            {state.activeCardEffects
              ...
              .map((e, i) => (
                <div
                  key={e.cardId + i}
                  className="flex items-center gap-3 text-xs text-zinc-400 bg-zinc-800/50 rounded px-3 py-2"
                >
                  <span className="font-mono text-zinc-500">{i + 1}.</span>
                  <span className="font-mono text-indigo-400">{e.cardId}</span>
                  <span className="text-zinc-600">•</span>
                  <span>
                    Alvo:{' '}
                    <span className="text-zinc-300">
                      {e.effect.targetFilter ?? e.effect.target}
                    </span>
                  </span>
                </div>
              ))}
